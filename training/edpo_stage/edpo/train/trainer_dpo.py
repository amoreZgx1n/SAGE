...
class EDPOTrainer(Trainer):
    ...
    def score_answer(self, text):
        prompt = f"""
        Please rate the following answer on a scale from 1 (very poor) to 5 (excellent), considering its factual accuracy, logical reasoning, and clarity.
        ...
        try:
            score = float(content.strip().split("\n")[0])
            score = max(1.0, min(5.0, score))
        except:
            score = 3.0
        return score

    def edpo_group_loss(self, batch_group):
        losses = []
        scores = []
        for batch in batch_group:
            chosen = batch['chosen']
            rejected = batch['rejected']
            chosen_score = self.score_answer(chosen)
            rejected_score = self.score_answer(rejected)
            diff = abs(chosen_score - rejected_score)
            scores.append((diff, batch, chosen_score, rejected_score))

        sorted_scores = sorted(scores[:3], key=lambda x: x[0])
        selected_pair = sorted_scores[0][1]
        AD_pair = batch_group[3]

        for b, use_grad in zip(batch_group, [b==selected_pair or b==AD_pair for b in batch_group]):
            loss = self.compute_loss(self.model, b, return_outputs=False)
            if use_grad:
                losses.append(loss)
            else:
                losses.append(loss.detach())

        total_loss = sum(losses) / 2
        return total_loss
    ...
    def training_step(self, model, inputs):
        ...
        batch_group = self.buffer.copy()
        loss = self.edpo_group_loss(batch_group)
        self.buffer.clear()
        return loss
