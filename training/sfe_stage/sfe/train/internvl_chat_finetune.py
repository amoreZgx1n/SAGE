...
import transformers
from sfe.dist_utils import init_dist
from sfe.model.internlm2.modeling_internlm2 import InternLM2ForCausalLM
from sfe.model.internvl_chat import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
from sfe.patch import (concat_pad_data_collator,
...
from sfe.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      IMG_CONTEXT_TOKEN, IMG_END_TOKEN,
                                      IMG_START_TOKEN, QUAD_END_TOKEN,
                                      QUAD_START_TOKEN, REF_END_TOKEN,
                                      REF_START_TOKEN)
from sfe.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_phi3)
from sfe.train.dataset_packed import PackedDataset, packed_collate_fn
...
from sfe.model.internvl_chat.graph_generate import generate_soft_prompt
...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        description_template = [
            "Surface anomaly detection is a technology used to identify defects on the surface of materials or products, widely applied in industrial manufacturing, semiconductor production, metal processing, food quality control, and other fields. Its purpose is to detect anomalies such as scratches, cracks, contamination, and deformations that may affect product performance, appearance, or safety, ensuring consistent production quality.",
            
            "The core objective of surface anomaly detection is to improve product quality, reduce defect rates in the production process, and minimize economic losses caused by defects. In high-precision industries such as semiconductor manufacturing and aerospace, even minor surface defects can impact functionality and reliability, making efficient anomaly detection crucial. Detecting and addressing surface defects promptly can enhance production efficiency, optimize material usage, and reduce waste and rework costs.",
            
            "Surface anomalies can be categorized into several types, including mechanical damage (scratches, cracks, dents), material defects (bubbles, holes, peeling), color abnormalities (discoloration, contamination, oxidation), and structural anomalies (warping, deformation, unevenness). These anomalies can result from production equipment, material properties, environmental conditions, or improper human operations and must be identified and classified for appropriate corrective actions.",
            
            "Different industries have established various standards for surface quality assessment. For example, the glass manufacturing industry follows ISO 10110-7 for surface imperfections, steel testing uses ASTM E45 for inclusion evaluation, electronic components follow IPC-A-600 for soldering defects, and automotive manufacturing may rely on VDA 16 for coating and surface assessment. These standards provide quantifiable criteria to ensure compliance with quality requirements.",
            
            "Traditional surface anomaly detection primarily relies on manual visual inspection, optical microscopy, and laser scanning. Manual inspection depends on operator experience but is prone to fatigue and subjective judgment errors, leading to inconsistent results. Optical inspection can magnify defects and improve visualization accuracy but is still limited by operational complexity and inspection speed.",
            
            "Modern surface anomaly detection increasingly leverages computer vision technologies, including high-resolution cameras, image processing algorithms, and deep learning models. These methods can automatically identify defects through edge detection, color analysis, and morphological operations, and classify anomalies using machine learning models, significantly enhancing detection accuracy and efficiency.",
            
            "Deep learning technologies, such as convolutional neural networks (CNNs), generative adversarial networks (GANs), and self-supervised learning (SSL), have made significant advancements in surface anomaly detection. These models learn features from large defect datasets, enabling automatic anomaly detection and classification across various tasks, such as metal surface scratch detection, PCB defect inspection, and glass bubble detection.",
            
            "In the context of Industry 4.0 and smart manufacturing, surface anomaly detection is increasingly integrated with automated production lines to achieve real-time inspection. Industrial cameras embedded in production lines can scan products one by one, and AI algorithms analyze images in real-time, identifying anomalies within milliseconds and triggering alerts or automatic defect rejection, greatly improving detection efficiency and production stability.",
            
            "Despite significant advancements, surface anomaly detection still faces numerous challenges. Variations in lighting conditions can affect image quality and lead to unstable detection results; complex materials such as highly reflective metals or transparent glass can complicate defect identification; and limited data availability may hinder deep learning model performance. Addressing these challenges requires a combination of data augmentation, adaptive thresholding, and multi-modal sensors.",
            
            "In the future, surface anomaly detection technology will become more intelligent, efficient, and automated. As deep learning and computer vision continue to evolve, detection systems will gain stronger generalization capabilities and real-time performance. Additionally, multi-modal integration (e.g., combining spectral analysis and ultrasonic testing), few-shot learning, and unsupervised anomaly detection will further drive advancements in precision and cost-effectiveness, contributing to the intelligent upgrade of industrial quality control."
        ]
        ...
        if len(lines) > max_lines:
            ...
            lines.extend(random.sample(description_template, needed))
        tokenized_lines = [self.tokenizer(line, return_tensors='pt', padding='max_length', truncation=True, max_length=256) for line in lines]
        description_ids = torch.stack([tokenized['input_ids'][0] for tokenized in tokenized_lines], dim=0)  # (num_lines, max_length)
